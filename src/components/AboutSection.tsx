...
export function AboutSection({ skills, details, socials }: AboutSectionProps) {
  return (
    <section id="about" aria-labelledby="about-alfian" className="py-10 md:py-14 px-6">
        ...
        <div className="flex gap-8 max-md:flex-col items-center md:items-start">
          ...
          <div className="flex flex-col gap-4 justify-between flex-1 min-w-0 w-full">
            <div>
              ...
              <h2 id="about-alfian" className="text-3xl max-sm:text-2xl font-bold mt-1 mb-3">
                About Me
              </h2>
              ...
              <div className="grid grid-cols-2 max-sm:grid-cols-1 gap-x-4 gap-y-3">
                {details.map((detail) => (
                  <div key={detail.label} className="min-w-0">
                    ...
                    {detail.isEmail ? (
                      <a
                        href={`mailto:${detail.value.replace("[at]", "@")}`}
                        className="mt-0.5 block truncate text-sm text-[var(--color-text-primary)] transition-colors hover:text-accent focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent"
                        title={detail.value}
                      >
                        {detail.value}
                      </a>
                      ...
                      <p
                        className="mt-0.5 truncate text-sm text-[var(--color-text-primary)]"
                        title={detail.value}
                      >
                        {detail.value}
                      </p>
