...
import { getAiProvider, OpenAiProvider, MockFallbackProvider } from "@/lib/ai/provider";
...
describe("AI Provider Selection & Factory", () => {
  ...
  it("prioritizes Nara AI Gateway when NARA_API_KEY is configured", () => {
  ...
  it("uses default Nara base URL and model if only NARA_API_KEY is given", () => {
    process.env.NARA_API_KEY = "test-nara-key";
  ...
  it("falls back to SumoPod AI Gateway when NARA_API_KEY is missing but SUMOPOD_API_KEY is present", () => {
    process.env.SUMOPOD_API_KEY = "test-sumopod-key";
  ...
  it("falls back to MockFallbackProvider in simulated mode when no API keys are set", () => {
    const { provider, mode } = getAiProvider();
